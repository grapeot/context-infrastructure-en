...
**The single hard test**: Read a paragraph out loud. If it sounds like reading from a script, if you have to put on a formal cadence, it is stiff. If it sounds like your normal speaking rhythm, it is right. This is more reliable than any word list; defer to it.

**After writing, self-check along these five tone dimensions** (not word-by-word avoidance; it is dimension alignment):
- **Subject granularity**: Is the subject of each sentence a specific person, company, or action? If a whole paragraph's subjects are abstract nouns (logic, mechanism, predicament, trend, game), the tone has gone stiff. Swap in concrete actors.
- **Word register**: Would you say this word out loud? "raked in, game, peddle, confirm, outmatched" are written words; "streamed, sold, handed them ammo, single-handed" are spoken words. Draw from the second layer.
- **Sentence length**: A sentence over 30 words with three modifiers is "writing," not "talking." Split it.
- **Subheadings**: Read one to a friend. Does it sound like a slide-deck section name ("In the vacuum of platform silencing, the dark threads of the game")? If so, rewrite it as a plain-spoken judgment ("The real reason he got banned may be hiding in himself").
- **Rhetoric density**: Ornamental metaphors ("fell under the muzzle of the rules," "precision strike," "asymmetric weapon"). If there are more than two or three in a piece, rhetoric is covering for thin content. Cut to only the necessary ones.

Why not a banned-word list: Gemini's stiffness is not a problem of individual words. It is that the model defaults to drawing from written corpus and defaulting to abstract nouns as subjects. Ban "raked in" and it swaps in "hauled in"; ban "game" and it swaps in "contest." A word list can never catch up with its vocabulary. Have it understand the target tone and these five dimensions, and it will systematically adjust its word register and sentence structure.
