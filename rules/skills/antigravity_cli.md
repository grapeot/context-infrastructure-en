...
- IC-3 reads the brief, IC-2 deliverable, calibration material, prose rules (with the positive sample and both negative extremes), then first judges the whole-article voice. If the opening, multiple H2 entries, and the ending still read like a lecture, the whole prose must be rewritten, not just word-swapped. Then check whether familiarity was added by introducing metaphors, slang, absolute conclusions, or new facts not in the source pack. Output `article_qa.md` and `prose_qa.md`; IC-3 is the external-facing workflow's final prose authority.
- The main thread then performs Manager Content Acceptance per `workflow_external_writing.md`: treat `article_qa.md` as the irreplaceable base document and make only word-level factual, naming, numeric, and claim-strength corrections. Direct edits may touch no more than 5% of non-whitespace body characters. A paragraph-level or structural change must go through `content_audit.md` and a fresh AGY targeted final-integration conversation; the main thread must not select, splice, or independently rewrite prose.
