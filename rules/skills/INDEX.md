...
This index only routes: it tells you which skill to read when, not how to invoke it. After a hit, read the corresponding skill file before acting.

- **To use a capability** → find the skill file in the categories below, then **read it**
- **To add a new skill** → follow the existing format and add to the matching category
...
- [Playwright Ajax Capture](./playwright_ajax_capture.md) — Intercept fetch/XHR in an authorized CDP browser session to reverse-engineer a web app's internal API protocol. Trigger words: "capture ajax", "reverse-engineer internal API", "browser session API call", "no admin key"
...
- **INDEX.md** provides routing only, no operational details
- **Specific skill files** contain complete operational steps, commands, and examples — read them after a hit
