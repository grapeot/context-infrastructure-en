...
| Agent operations | [ai-agent-cli-skill](https://github.com/grapeot/ai-agent-cli-skill) | File-based non-interactive calls to Claude Code, Codex, OpenCode, Antigravity, and Grok Build; expose one root skill and load per-CLI files on demand |
| Agent authentication | [chat-gpt-oauth-skill](https://github.com/grapeot/chat-gpt-oauth-skill) | Users bring their own ChatGPT Plus/Pro subscription and authorize manually on localhost; includes browser PKCE, plaintext token lifecycle, refresh, and a minimal Codex Responses example. Owner-only experiment; the compatibility endpoint is unstable and not recommended for production |
| Agent authentication | [grok-oauth-skill](https://github.com/grapeot/grok-oauth-skill) | Users bring their own SuperGrok / X Premium+ subscription and authorize manually; includes browser PKCE, device code, plaintext token lifecycle, refresh, and a minimal xAI Chat Completions example. Owner-only experiment; the compatibility endpoint is unstable and not recommended for production |
...
| Usage analytics | [ai-session-profanity-rate](https://github.com/grapeot/ai-session-profanity-rate) | Count profanity units in human-authored local AI-session messages with batched sub-agents, a versioned cache, redacted JSON, daily incidence, and model-composition charts; transcripts and generated results stay local |
...
| 3D / animation | [gpt_3d_skill](https://github.com/grapeot/gpt_3d_skill) | Created specifically to put GPT-6's leap in 3D modeling to work: reference breakdown, materials, camera choreography, and iterative visual checks help move beyond rough demos toward deliverable Blender models, animations, and Three.js walkthroughs. Includes character rigging and local motion capture workflows |
| Music | [zun-music-skill](https://github.com/grapeot/zun-music-skill) | Arrange a public-domain tune as a short ZUN-style (Touhou Project) phrase, typically 8 bars / ~15 s: the source's strong-beat skeleton is kept and automatically checked while ZUN progressions, 3-3-2 syncopation, 16th-note runs, a piano-doubled trumpet lead, and programmed-sounding drums go on top; renders MIDI/MP3 with FluidSynth and a Touhou-oriented SoundFont and serves a LAN listening page for A/B comparison. Includes a public-domain check and pitfalls from real listening rounds |
| Video | [opus-video-audio-skill](https://github.com/grapeot/opus-video-audio-skill) | Designed for and validated with Claude Opus: render short videos frame by frame from code and score them to picture. The video side covers settling the concept before rendering, choosing the focal length from real angular size, linear compositing with a single tone map, glow / reflection / on-screen-text pitfalls, and frame review that never inspects a stale pass; the audio side uses MIDI + FluidSynth so notes land on picture beats and, because an agent cannot hear, verifies duration, peak, RMS envelope, and spectral centroid objectively. Ships `check_frames.py` and `score_cue.py`; untested on other models |
